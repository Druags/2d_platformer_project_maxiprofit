...
def create_player():  # функция, создающая игрока
    ...
    rect_player = img_player.get_rect(topleft=(100, 100))  # из image создаём прямоугольник(управляемый объект)


def update_player(screen, world):  # фукнция для обновления состояния игрока
    global img_player, rect_player, index, counter, direction, jumped, gravity  # получаем доступ к переменным,
    ...
    new_rect_player_dx = rect_player.copy()
    new_rect_player_dx.x += dx
    new_rect_player_dy = rect_player.copy()
    new_rect_player_dy.y += dy
    # pygame.draw.rect(screen, 'red', new_rect_player_dx) # хитбокс по иксу
    # pygame.draw.rect(screen, 'green', new_rect_player_dy) # хитбокс по игреку
    jumped = True
    for tile in world:  # перебираем список
        tile = pygame.Rect(*tile[1], 40, 40)  # создаём ощутимый блок
        if tile.colliderect(new_rect_player_dx):  # проверяем столкновение по иксу
            dx = 0  # персонаж никуда не движется по иксу
        if tile.colliderect(new_rect_player_dy):  # проверяем столкновение по игреку
            if gravity > 0:  # если падаем вниз
                dy = tile.top - rect_player.bottom
                jumped = False
            else:  # если стоим или прыгаем
                dy = tile.bottom - rect_player.top
                gravity = 0
